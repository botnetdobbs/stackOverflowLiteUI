...
const api = new API(Auth.getAccessToken());
const ui = new UI();
const auth = new Auth();
//Get the user object
...
/**
 * Fetch all questions when the DOM loads
 */
...
/**
 * Get a detailed info when a question is clicked
 */

//Get the wrapper for all the questions
const singleQuestion = document.getElementById("all-questions");
//Add an event listener in side the whole wrapper
singleQuestion.addEventListener("click", getaQuestion);

//This is an event listener for a click on a question on the logged in user's profile page.
...
/**
 * @param {*} e - this is the click event object
 * @description This function gets a single question
 */
function getaQuestion(e) {
  ...
  if (target.parentElement.className === "description") {
    //Get the question
    api.get(target.getAttribute("href")).then(data => {
      if (data.message) {
        if (Auth.getAccessToken()) {
          ui.loadUserItems();
        }
        return ui.loadMessage(data.message, "error");
      } else {
    ...
    api.get(`${target.getAttribute("href")}/answers`).then(data => {
      if (data.message) {
        //Answers not found
        answers = [];
      } else {
  ...
  // Since the above promises are asynchronous, the code below will run before them hence the setTimeout
  setTimeout(() => {
    //Actions
    ui.loadQuestion(question, user.username);
    ui.loadAnswers(answers, question, user.username);
    ui.loadAnswerForm();
    //If user is logged in render the user items
    if (Auth.getAccessToken()) {
      ui.loadUserItems();
...
/**
 * Do actions on answers when they're clicked i.e upvote, downvote
 */
 ...
 * 
 * @param {*} e Click event
 * @description This funciton returns the login, register, logout or profile page depending on user options
 */
function renderAuth(e) {
  ...
  if (target.parentElement.classList.contains("register")) {
    ...
    //Get questions for the currently logged in user
    api.get('/user/questions').then(data => {
...
/**
 * @description this function handles user registration
 */
...
//       "error",
//       document.querySelector(".login-reg")
//     );
//     return false;
...
//     ui.loadMessage(
//       "The username must not be less than 3  or more than 18 characters",
//       "error",
//       document.querySelector(".login-reg")
//     );
//     return false;
...
/**
 * @description Event listeners and action for the profile page
 */
...
function mainPage(e) {
  ...
  if (target.parentElement.classList.contains("post-question")) {
     ...
     */
    const url = target.getAttribute("href");
    myApi.delete(url, user.access_token).then(data => {
    ...
    const url = target.getAttribute("href");
    myApi.getQuestion(url).then(data => {
...
function postQuestion() {
  ...
  if (title === "") {
    ...
    ui.loadMessage(
      "The title must not be less than 3  or more than30     characters",
      "error",
      document.querySelector("#form-wrapper")
    );
    return false;
  ...
  if (description === "") {
    ...
    ui.loadMessage(
      "The description must not be less than 3  or more than 300     characters",
      "error",
      document.querySelector("#form-wrapper")
    );
    return false;
    ...
    ui.loadMessage(
      "The description must begin with a letter",
      "error",
      document.querySelector("#form-wrapper")
    );
    return false;
  ...
  myApi.postQuestion(question, user.access_token).then(data => {
...
function updateQuestion() {
  ...
  myApi.updateQuestion(question, question_id, user.access_token).then(data => {
    if (data.id) {
...
function postAnswer() {
  ...
  if (answer === "") {
    ...
    ui.loadMessage(
      "The answer must not be less than 3  or more than 255     characters",
      "error",
      document.querySelector("#form-wrapper")
    );
    return false;
    ...
    ui.loadMessage(
      "The answer must begin with a letter",
      "error",
      document.querySelector("#form-wrapper")
    );
    return false;
  ...
  myApi
    .postAnswer(questUrl + "/answers", thee_answer, user.access_token)
    .then(data => {
      console.log(data);
      if (data.message === "Answer inserted successfully") {
        ...
        setTimeout(() => {
          ...
          myApi.getQuestion(questUrl).then(data => {
